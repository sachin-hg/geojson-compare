...
- **Formula:** ```{name: region_entity_name, href: getCanonical(region_entity_id) || "", type: region_entity_type}```, ```{name: short_address.map(x => x.display_name).join(', '), href: getCanonical(short_address.findLocality(x => isLocality(x.polygon_uuid))), type: "locality"}```. Filter nulls by ```name``` only.
